...
# "model_path": "/caa/Homes01/wmucha/repos/applied_deep_learning/applied_dl/custom_heatmap_allaug5_61",
    ...
    if MODEL_TYPE == EgocentricModelType.poseresnet50:
        ...
        model = init_model(config_file, checkpoint_file,
                           device=DEVICE)  # or device='cuda:0'
    ...
    elif MODEL_TYPE == EgocentricModelType.mediapipe:
...
if __name__ == "__main__":
    ...
    # make argument parser to read given arguments from command line
    arg_parser = argparse.ArgumentParser()
    arg_parser.add_argument("--model_type", type=str, default="effhandegonet",
                            help="Type of model to use for predictions(effhandegonet, effhandnet, mediapipe, poseresnet50)")
    arg_parser.add_argument("--device", type=int, default='cuda',
                            help="Device to use for predictions")
    arg_parser.add_argument("--data_type", type=str, default="test",
                            help="Type of dataset: train, val, test")
    arg_parser.add_argument("--save_pose", type=bool, default=False,
                            help="Save the predicted poses to file or not; True/False"),
    arg_parser.add_argument("--save_yolo_objects", type=bool, default=False,
                            help="Save the YOLO objects to file or not; True/False"),
    arg_parser.add_argument("--dataset_path", type=str,
                            default="/data/wmucha/datasets",)

    args = arg_parser.parse_args()

    MODEL_TYPE = EgocentricModelType[args.model_type]
    DATA_TYPE = args.data_type
    DEVICE = args.device
    SAVE_PREDICTED_POSE = args.save_pose
    SAVE_YOLO_OBJECTS = args.save_yolo_objects
